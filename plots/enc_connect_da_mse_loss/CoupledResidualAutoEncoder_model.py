...
class CoupledResidualAutoEncoder(nn.Module):
    ...
    def __init__(self, encoded_dim=256):
        ...
        self.lin = nn.Linear(4*encoded_dim, encoded_dim)
        self.relu = nn.ReLU()

        hidden_dims.reverse()
        self.da_encoder = DAEncoder(hidden_dims, 3, encoded_dim)
        self.trg_decoder = Decoder(hidden_dims, encoded_dim)

    def forward(self, x, x_trg):
        x_src, enc_outs = self.encoder.forward(x)
        x_trg, da_enc_outs = self.da_encoder.forward(x_trg, enc_outs)

        da_enc_outs.append(x_src)
        da_enc_outs.append(x_trg)
        da_enc_outs = torch.cat(da_enc_outs, 1)

        x_da = self.lin(da_enc_outs)
        x_da = self.relu(x_da)
